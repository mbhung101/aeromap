...
When looking at the dataset, I confess I had no idea how to approach it. The dataset didn't look like any I'd seen before so I did some research on what kind of visualization would even be able of representing the data. After learning a little about graph theory, I found an appropriate tool kit in vis-js that had been ported to React, my frontend framework of choice. Vis js specializes in network visualizations and had a fairly simple customization for nodes/edges.

In terms of actual design choice, there certainly was alot to be represented. I opted to utilize labels as little as possible and use a color scheme with a sidepanel key. Edges only required one color, since all the listed types were the same, I didn't feel the need to represent that. Nodes needed a lot of characterization so just as a placeholder given the limited time I had, I decided to have the broader object classification as the fill color and the border to denote the subtypes.
